...
def agent_portrayal(agent):
    ...
    if isinstance(agent, Firm):
        ...
        if pos is None:
            return (
                AgentPortrayalStyle()
            )  # Return empty style for agents without position
        ...
        return AgentPortrayalStyle(
            ...
            color=color,
            size=50,
            tooltip={"Firm ID": agent.firm_id, "Position": pos, "Profit": agent.profit},
        )
    return AgentPortrayalStyle()
...
try:
    ...
    def Page():
        ...
        solara.Markdown(
            "**Visualization dependencies missing. Install with:** `pip install mesa[viz]`"
        )
        ...
        with solara.Column():
            ...
            for agent in model.agents:
                ...
                if pos:
                    solara.Markdown(
                        f"- **Firm {agent.firm_id}**: Position {pos}, Profit {agent.profit}"
                    )
                else:
