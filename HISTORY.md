...
Several optimizations improve Mesa's performance: `PropertyLayer` was refactored to implement the NumPy interface directly, enabling standard NumPy syntax and deprecating wrapper methods (#3074); `select_random_empty_cell()` now uses vectorized NumPy operations instead of slow O(N) Python iteration (#3087); and `Cell.is_empty`/`is_full` checks were optimized to remove unnecessary O(n) copies (#3069).
