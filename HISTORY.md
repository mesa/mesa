...
This release resolves several bugs affecting simulation accuracy: fixed multiple `batch_run` and `DataCollector` issues including `agenttype_reporters` support (#3095), sparse data collection (#2988), and proper handling of multiple `collect()` calls per step (#3058); resolved `MultiGrid._empty_mask` not updating correctly (#3019) and infinite loops in `select_random_empty_cell()` (#3014); fixed a memory leak in `ContinuousSpace` agent removal (#3031); and corrected `EventList.peek_ahead()` chronological ordering (#3010).
