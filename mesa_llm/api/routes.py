...
from .models import (
    GenerateSimulationRequest, GenerateSimulationResponse,
    DebugCodeRequest, DebugCodeResponse,
    ExplainCodeRequest, ExplainCodeResponse,
    OptimizeCodeRequest, OptimizeCodeResponse,
...
from ..utils import config, logger, LLMProvider, validate_config

# Create router
router = APIRouter()

# Dependency to get default LLM provider
def get_llm_provider(provider: Optional[LLMProvider] = None) -> LLMProvider:
...
async def generate_simulation(
    ...
    try:
        logger.info(f"Generating simulation with provider: {request.llm_provider or llm_provider}")
...
async def execute_code(request: ExecuteCodeRequest):
    ...
    try:
        ...
        result = executor.run_simulation_steps(
            code=request.code,
            steps=request.steps,
            collect_data=request.collect_data
        )
...
async def generate_simulation_stream(
    ...
    try:
        ...
        provider = LLMProviderFactory.create_provider(request.llm_provider or llm_provider)
        prompt_manager = PromptManager()
        
        messages = [
            provider.create_system_message(prompt_manager.get_system_prompt(TaskType.GENERATE)),
            provider.create_user_message(
                prompt_manager.get_generation_prompt(request.prompt, request.simulation_type)
            )
        ]
