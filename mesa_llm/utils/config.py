...
class Config(BaseSettings):
    ...
    allowed_imports: list[str] = Field(default=[
        "mesa", "numpy", "pandas", "matplotlib", "random", "math", 
        "collections", "itertools", "enum", "typing", "dataclasses"
    ])
...
def validate_config() -> None:
    ...
    if not config.openai_api_key and not config.gemini_api_key:
        errors.append("At least one LLM API key must be provided (OPENAI_API_KEY or GEMINI_API_KEY)")
