...
from ..prompts import PromptManager, TaskType, SimulationType
...
class MesaCodeGenerator:
    ...
    async def generate_simulation(
        ...
        try:
            ...
            messages = [
                ...
                LLMMessage(
                    role="user", 
                    content=self.prompt_manager.get_generation_prompt(user_prompt, simulation_type)
                )
            ]
            ...
            if validate_code:
                ...
                if not validation_results["is_valid"]:
                    logger.warning(f"Generated code validation failed: {validation_results['errors']}")
    ...
    async def generate_agent_class(
        ...
        base_class: str = "mesa.discrete_space.CellAgent"
    ) -> str:
...
Generate only the agent class code:"""
        ...
        messages = [
            LLMMessage(role="system", content=self.prompt_manager.get_system_prompt(TaskType.GENERATE)),
    ...
    def _validate_mesa_patterns(self, tree: ast.AST, code: str) -> Dict[str, Any]:
        ...
        for agent_class in agent_classes:
            if 'step' not in agent_class['methods']:
                validation["warnings"].append(f"Agent class {agent_class['name']} missing step() method")
        ...
        for model_class in model_classes:
            if 'step' not in model_class['methods']:
                validation["warnings"].append(f"Model class {model_class['name']} missing step() method")
