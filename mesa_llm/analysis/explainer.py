...
class MesaExplainer:
    ...
    async def explain_simulation(
        self, code: str, focus_area: str | None = None, audience_level: str = "beginner"
    ) -> dict[str, Any]:
        ...
        """
        logger.info(
            f"Generating explanation for simulation (audience: {audience_level})"
        )
    ...
    def _analyze_code_structure(self, code: str) -> dict[str, Any]:
        ...
        try:
            ...
            analysis["data_collection"] = (
                "DataCollector" in code or "datacollector" in code
            )
    ...
    async def _generate_llm_explanation(
        ...
        instruction = audience_instructions.get(
            audience_level, audience_instructions["beginner"]
        )
...
"""
        ...
        messages = [
            LLMMessage(
                role="system",
                content=self.prompt_manager.get_system_prompt(TaskType.EXPLAIN),
            ),
    ...
    def _create_structured_explanation(
        self, code_analysis: dict[str, Any], llm_explanation: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        structured = {
            ...
            "environment": self._create_environment_explanation(
                code_analysis["spatial_components"]
            ),
    ...
    def _create_agent_explanations(
                    ...
                    explanation["key_methods"].append(
                        {
                            ...
                            "description": method.get(
                                "docstring", f"Handles {method['name']} behavior"
                            ),
                        }
                    )
    ...
    def _create_model_explanation(
        ...
        for method in model["methods"]:
            if method["name"] in method_responsibilities:
                explanation["responsibilities"].append(
                    method_responsibilities[method["name"]]
                )
                explanation["key_methods"].append(
                    {
                        "name": method["name"],
                        "description": method.get(
                            "docstring", method_responsibilities[method["name"]]
                        ),
                    }
                )
    ...
    def _create_environment_explanation(
        self, spatial_components: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """Create explanation for environment/space."""
        if not spatial_components:
            return {
                "type": "No spatial environment",
                "description": "Agents exist without spatial relationships",
            }
