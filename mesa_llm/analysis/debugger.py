...
class MesaDebugger:
    ...
    async def debug_code(
        self, code: str, error_message: str | None = None, run_tests: bool = True
    ) -> dict[str, Any]:
        ...
        llm_analysis = await self._llm_debug_analysis(
            code, error_message, static_analysis, execution_analysis
        )
        ...
        return {
            ...
            "summary": self._create_debug_summary(
                static_analysis, execution_analysis, llm_analysis
            ),
        }
    ...
    def _static_code_analysis(self, code: str) -> dict[str, Any]:
        ...
        try:
            ...
            analysis["best_practice_violations"] = self._check_best_practices(
                tree, code
            )
    ...
    async def _llm_debug_analysis(
        ...
        messages = [
            LLMMessage(
                role="system",
                content=self.prompt_manager.get_system_prompt(TaskType.DEBUG),
            ),
    ...
    def _format_analysis_for_llm(self, analysis: dict[str, Any]) -> str:
        ...
        for key, value in analysis.items():
            ...
            elif (isinstance(value, dict) and value) or value:
    ...
    def _create_debug_summary(
        ...
        if summary["critical_issues"] > 0:
            summary["recommendations"].append(
                "Fix critical errors before running simulation"
            )
        ...
        if not summary["is_valid_mesa"]:
            summary["recommendations"].append(
                "Ensure code creates a valid Mesa simulation"
            )

        if summary["total_issues"] == 0:
            summary["recommendations"].append(
                "Code looks good! Consider running performance optimization"
            )
