...
class MesaLLMClient:
    ...
    def debug_code(
        ...
        data = {"code": code, "llm_provider": llm_provider, "run_tests": run_tests}
        if error_message:
    ...
    def optimize_code(
        self, code: str, focus_areas: list = None, llm_provider: str = "openai"
    ) -> dict[str, Any]:
        """Optimize Mesa simulation code."""
        data = {"code": code, "llm_provider": llm_provider}
        if focus_areas:
    ...
    def execute_code(
        self, code: str, steps: int = 10, collect_data: bool = True
    ) -> dict[str, Any]:
        """Execute Mesa simulation code."""
        data = {"code": code, "steps": steps, "collect_data": collect_data}
...
def example_workflow():
    ...
    if generation_result["success"]:
        ...
        explanation_result = client.explain_code(
            code=generated_code, audience_level="beginner"
        )

        if explanation_result["success"]:
            print("✅ Explanation generated!")
            print(
                "Overview:", explanation_result["explanation"]["overview"][:200] + "..."
            )

        print("\n🔧 Optimizing the code...")
        optimization_result = client.optimize_code(
            code=generated_code, focus_areas=["performance", "readability"]
        )
        ...
        if optimization_result["success"]:
            ...
            opportunities = optimization_result["optimization_report"][
                "total_opportunities"
            ]
            print(f"Found {opportunities} optimization opportunities")

        print("\n▶️ Executing the simulation...")
        execution_result = client.execute_code(code=generated_code, steps=5)
        ...
        if execution_result["success"]:
            ...
            debug_result = client.debug_code(
                code=generated_code, error_message=execution_result["error"]
            )
