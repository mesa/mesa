...
def test_create_meta_agent_independent_groups_with_overlap():
    ...
    assert len(agent1.meta_agents) == 2
    assert (group1_in_meta := any(ma.__class__.__name__ == "GroupA" for ma in agent1.meta_agents))
    assert (group2_in_meta := any(ma.__class__.__name__ == "GroupB" for ma in agent1.meta_agents))
