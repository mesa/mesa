...
def test_rng(rng=23):
    ...
    model = Model(rng=rng)
    assert model.scenario.initial_rng_state == np.random.default_rng(rng).bit_generator.state
    model2 = Model(rng=rng + 1)
    assert model2.scenario.initial_rng_state == np.random.default_rng(rng + 1).bit_generator.state
    assert model.scenario.initial_rng_state == np.random.default_rng(rng).bit_generator.state
