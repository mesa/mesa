...
def test_agent_instance_is_hashable():
    """Test that a standard Agent instance is hashable."""
    model = Model()
    agent = Agent(model)
    test_set = {agent}
    assert agent in test_set


def test_unhashable_agent_subclass():
    """Test that an Agent subclass that overrides __eq__ but not __hash__ is unhashable."""

    class MockModel(Model):
        def register_agent(self, agent):
            pass

    class UnhashableAgent(Agent):  # noqa: PLW1641
        def __eq__(self, other):
            return self.unique_id == other.unique_id

    model = MockModel()
    agent = UnhashableAgent(model)
    with pytest.raises(TypeError, match="unhashable type: 'UnhashableAgent'"):
        hash(agent)


def test_hashable_agent_subclass():
    """Test that an Agent subclass that overrides both __eq__ and __hash__ is hashable."""

    class HashableAgent(Agent):
        def __eq__(self, other):
            return (
                isinstance(other, HashableAgent) and self.unique_id == other.unique_id
            )
