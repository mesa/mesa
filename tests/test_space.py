...
class TestSingleGrid(unittest.TestCase):  # noqa: D101
    ...
    def test_move_agent_closest_hybrid_small_set(self):
        ...
        agent.pos = (5, 5)
        possible_positions = [
            (5 + i, 5 + j) for i in range(-2, 3) for j in range(-2, 3)
        ]

        self.space.move_agent_to_one_of(agent, possible_positions, selection="closest")

        min_dist = min(
            self.space._distance_squared(p, (5, 5)) for p in possible_positions
        )
        closest_positions = [
            p
    ...
    def test_move_agent_closest_hybrid_large_set(self):
        ...
        min_dist = min(
            self.space._distance_squared(p, (25, 25)) for p in possible_positions
        )
        closest_positions = [
            p
            for p in possible_positions
