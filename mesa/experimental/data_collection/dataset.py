...
class AgentDataSet[A: Agent](BaseDataSet):
    ...
    def data(self) -> list[dict[str, Any]]:
        ...
        results = []
        for agent in agents:
            values = self._collector(agent)
            # FIX: operator.attrgetter returns a scalar if only 1 attribute is requested.
            # zip requires iterables, so we must wrap the scalar in a tuple.
            if len(self._attributes) == 1:
                values = (values,)
            results.append(dict(zip(self._attributes, values)))
