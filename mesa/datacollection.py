...
# Types that are deeply immutable and do not need deepcopying.
# Note: tuple is excluded as it can hold mutable objects (e.g., lists).
_PYTHON_IMMUTABLE_TYPES = (str, int, bool, float, type(None))
...
class DataCollector:
    ...
    def add_table_row(self, table_name, row, ignore_missing=False):
        ...
        prepared_values = [
            value if isinstance(value, _PYTHON_IMMUTABLE_TYPES) else deepcopy(value)
            for value in (row.get(column) for column in table)
        ]

        for column, value in zip(table, prepared_values, strict=True):
